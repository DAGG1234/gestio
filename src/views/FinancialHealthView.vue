...
<template>
  <div class="flex-1 flex flex-col h-full overflow-hidden bg-slate-100">
    ...
    <main class="flex-1 overflow-y-auto p-4 sm:p-6 flex flex-col items-center justify-center relative custom-scrollbar">
      ...
      <div v-if="showSetupModal" class="fixed inset-0 z-50 bg-slate-950/70 backdrop-blur-md flex items-center justify-center p-4">
        <div class="bg-white rounded-3xl p-6 max-w-sm w-full shadow-2xl border border-slate-100 flex flex-col items-center text-center animate-in fade-in zoom-in duration-200">
          ...
          <p class="text-xs text-slate-500 font-medium mt-1 mb-5">Tu perrito guardián te acompañará a cuidar tus finanzas, {{ username }}. ¿Cómo te gustaría llamarlo?</p>
      ...
      <!-- Contenedor general -->
      <div class="w-full max-w-sm flex flex-col items-center justify-center gap-3 my-auto">
        
        <!-- Barra de salud de la mascota -->
        <div class="w-full bg-white/90 backdrop-blur-sm border border-slate-200/80 rounded-2xl px-4 py-3 shadow-xs flex flex-col gap-2">
